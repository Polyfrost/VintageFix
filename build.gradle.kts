...
val accessTransformerName = "vintagefix1${platform.mcMinor}_at.cfg"
  ...
  if (project.platform.isLegacyForge) {
    runConfigs {
      "client" {
        ...
        property("fml.coreMods.load", "org.embeddedt.vintagefix.core.VintageFixCore") // Loads the VintageFix core mod
      }
    }
  }
...
val googleaccess by sourceSets.creating
val googleimpl by sourceSets.creating

// Configures the output directory for when building from the `src/resources` directory.
sourceSets {
  main {
    googleaccess.compileClasspath += compileClasspath
...
dependencies {
  ...
  shade("com.esotericsoftware:kryo:5.1.1")
}
  ...
  jar {
    ...
    if (platform.isLegacyForge) {
      manifest.attributes += mapOf(
        ...
        "FMLAT" to accessTransformerName, // We want to use our access transformer
